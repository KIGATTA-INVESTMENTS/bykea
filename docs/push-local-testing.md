...
1. Create a free Supabase project of your own. Run the migrations in
   `supabase/` against it (they are plain SQL; the SQL editor accepts them).
2. Run `supabase/seed_test_driver.sql` there. It creates
   `testdriver@bykea.test` / `TestDriver123!`, approved, deposit paid.
